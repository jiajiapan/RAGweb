...
reranker = CrossEncoder(os.getenv("RERANK_MODEL"), device = "cpu") # get an rerank model, which will give a score to (query, chunk)
...
def retrieve(vs_name, query, k = 25, rerank = False, top_k = 5): # return the actual text, if not rerank, just k results, if rerank, top_k results 
    try:
        ...
        # Sigmoid, turn [-..., ...] to [0,1] 
        if rerank:
            chunks = semantic_search.to_pandas().reset_index(drop = True) # turn results into pandas dataframe, where old index is lost and 0,1,2... new index is set
            query_chunk_comb = [[query, chunk] for chunk in chunks[TEXT_COLUMN]] # [[query, chunk1], [query, chunk2], ...]
            chunks["_distance_reranked"] = reranker.predict(query_chunk_comb, activation_fct = Sigmoid())
