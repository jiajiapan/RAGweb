...
def embedding(pdf_path: str):
    ...
    vs_hash = hashlib.sha256(f"{SENTS_EMBED_MODEL}_{NUM_SUB_VEC}".encode()).hexdigest()
    vs_name = f"vs_{vs_hash}"
    tbl = db.create_table(
        vs_name, schema=schema, mode="overwrite"
    )  # "overwrite" means if there is a table with the same name, replace it
    ...
    for i in tqdm(range(0, int(np.ceil(len(sentences) / BATCH_SIZE)))):
        try:
            batch = [
                sent for sent in sentences[i * BATCH_SIZE : (i + 1) * BATCH_SIZE] if len(sent) > 0
            ]
            ...
            KeyboardInterrupt
        ):  # this means "ctrl + C", raise means keeping this exception and ending the task
            raise
